...
class DoctorController extends Controller
{
    ...
    public function apiStoreReview(Request $request, $doctor_id)
    {
        // 1. Validation (Laravel handles 422 JSON response automatically for AJAX/API)
        $request->validate([
        ...
        // 2. Business Logic Check: Prevent duplicate review
        $exists = DoctorReview::where('doctor_id', $doctor->id)
            ->where('email', $request->email)
            ->exists();

        if ($exists) {
            // Return 409 Conflict status for duplicate data
            return response()->json([
                'message' => 'You have already submitted a review for this doctor.',
            ], 409);
        }

        // 3. Image Upload Logic (Using basic move logic, consider Storage::put for production)
        $imagePath = null;
        ...
        return response()->json([
            ...
            'review_id' => $review->id,
            // Optionally return the new review data: 'review' => $review 
        ], 201);
    }
