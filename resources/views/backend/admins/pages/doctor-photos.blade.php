...
@push('title')
    <title>Doctor Photos Gallery | {{ env('APP_NAME') }}</title>
@endpush
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Photos Gallery for {{ $doctor->name }}</h4>
                            ...
                            <div class="row mb-4">
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <form action="{{ route('admins.doctors.photos.store', $doctor->id) }}" method="POST" enctype="multipart/form-data" id="uploadForm">
                                                @csrf
                                                ...
                                                <div class="photo-upload-area" id="uploadArea">
                                                    ...
                                                    <small class="form-text text-muted d-block mt-2">
                                                        Maximum 5MB per photo. Supported formats: JPEG, PNG, JPG, GIF, WEBP
                                                    </small>
                                                </div>
                                    ...
                                    <div class="card">
                                        <div class="card-header">
                                            <h5>
                                                ...
                                                <span class="badge badge-primary">{{ $doctor->photos->count() }}</span>
                                            </h5>
                                            <div class="card-header-action">
                                                ...
                                                <div class="gallery-grid" id="photosGrid">
                                                    ...
                                                    <div class="photo-card" data-id="{{ $photo->id }}">
                                                        ...
                                                        <img src="{{ asset($photo->photo_path) }}" 
                                                             alt="{{ $photo->caption ?? 'Doctor Photo' }}" 
                                                             class="photo-img"
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#saveOrderBtn').click(function() {
            ...
            $.ajax({
                url: "{{ route('admins.doctors.photos.order', $doctor->id) }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        $(document).on('click', '.set-primary-btn', function() {
            ...
            $.ajax({
                url: "{{ route('admins.doctors.photos.primary', [$doctor->id, ':photoId']) }}".replace(':photoId', photoId),
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}"
                },
                success: function(response) {
                    // handle both response.success and response.message patterns
                    const ok = response.success || response.message;
                    if (ok) {
                        ...
                        showAlert('success', response.message || 'Primary photo set.');
                    }
                },
                error: function(xhr) {
        ...
        $(document).on('click', '.toggle-status-btn', function() {
            ...
            $.ajax({
                url: "{{ route('admins.doctors.photos.toggle', [$doctor->id, ':photoId']) }}".replace(':photoId', photoId),
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}"
                },
                success: function(response) {
                    ...
                    if (typeof status !== 'undefined') {
                        ...
                        showAlert('success', response.message || 'Photo status updated.');
        ...
        $('#saveCaptionBtn').click(function() {
            ...
            $.ajax({
                url: "{{ route('admins.doctors.photos.caption', [$doctor->id, ':photoId']) }}".replace(':photoId', currentPhotoId),
                type: "PUT",
                data: {
                    _token: "{{ csrf_token() }}",
                    caption: caption
                ...
                success: function(response) {
                    if (response.success || response.message) {
                        ...
                        showAlert('success', response.message || 'Caption updated.');
                    }
                },
                error: function(xhr) {
        ...
        $(document).on('click', '.delete-photo-btn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{ route('admins.doctors.photos.delete', [$doctor->id, ':photoId']) }}".replace(':photoId', photoId);
                }
            });
        });
